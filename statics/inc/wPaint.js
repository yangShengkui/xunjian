...
(function($)
{
	...
	$.fn.wPaint.defaultSettings = {
		...
		menu 				 : ['undo', 'redo'], // menu items - appear in order they are set
									//工具
									//,'clear' ,'rectangle','ellipse',,'line','text','fillColor','lineWidth','strokeColor','eraser','pencil'
	...
	{	
		...
		setImage: function(data, addUndo)
		{
			...
			myImage.src = data.toString();//图片路径
			_self.ctx.clearRect(0, 0, _self.canvas.width, _self.canvas.height);
			$(myImage).load(function(){

				var o = getPhotoOrientation();//o为6时ios旋转90
				if(o == 6){
                    _self.ctx.save();
                    _self.ctx.translate(_self.canvas.width,  0);
                    _self.ctx.rotate(Math.PI / 2);
                    _self.ctx.drawImage(myImage, 0, 0, _self.canvas.height, _self.canvas.width);
                    _self.ctx.restore();
				}else{
